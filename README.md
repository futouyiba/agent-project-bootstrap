...
- 三个配套安装的 Codex/ChatGPT/Claude Code Skills：项目工作流 Bootstrap、单 Issue 交付闭环与单 PR 自动闭环；
- 新项目和存量项目的交互式初始化流程；
...
- 全局“合并收尾”意图，以及 Codex CLI/IDE 的 `/prompts:integrate`、Claude Code 的 `/integrate` 快捷入口；
...
上面的命令默认安装到 Codex/ChatGPT。安装到 Claude Code 时加上 `--target claude`（PowerShell 用 `-Target claude`）：

```sh
curl -fsSL https://raw.githubusercontent.com/futouyiba/agent-project-bootstrap/main/install.sh | sh -s -- --target claude --with-global-rule
```

```powershell
$installer = irm https://raw.githubusercontent.com/futouyiba/agent-project-bootstrap/main/install.ps1; & ([scriptblock]::Create($installer)) -Target claude -WithGlobalRule
```

默认安装到 Codex/ChatGPT 时，安装器把 `agent-project-bootstrap`、`agent-issue-loop` 和 `agent-pr-loop` 写入 `$CODEX_HOME/skills`，把快捷 Prompt 写入 `$CODEX_HOME/prompts/integrate.md`；未设置 `CODEX_HOME` 时使用 `~/.codex`。安装到 Claude Code 时，三个 Skills 写入 `~/.claude/skills`，slash 命令 `/integrate` 写入 `~/.claude/commands/integrate.md`，全局规则写入 `~/.claude/CLAUDE.md`；未设置 `CLAUDE_CONFIG_DIR` 时使用 `~/.claude`。如果已经安装旧版 Skill，各旧目录会先被重命名为带时间戳的备份；不同内容的同名 Prompt 或命令也会先备份。再次使用 `--with-global-rule` 会升级这个项目自己管理的规则块，不会覆盖其他全局说明。
...
安装后如果 Skill 没有立刻出现，重启 ChatGPT/Codex；安装到 Claude Code 时则重启 Claude Code。

## 第一次初始化项目

1. 确保当前目录已经是 Git 仓库。这个 Skill 不会未经允许替你初始化 Git。
2. 在 ChatGPT Work 中输入 `@agent-project-bootstrap`，或在 Codex 中输入 `$agent-project-bootstrap`。Claude Code 中该 Skill 会自动可用，直接用自然语言描述需求即可触发。
...
安装 GitHub 插件只是提供操作能力；安装 Skills 只是让 agent 理解流程。第一次对每个仓库执行 bootstrap，才会写入仓库规则、模板和检测标记。GitHub Project 的字段与自动化也需要针对该 Project 单独配置。

## Bootstrap Skill 的两种模式
...
`搞定 PR`、`搞定当前 PR` 或显式调用 `agent-pr-loop` 用于一个具体 PR。配套 Skill 会读取完整评论与 review，执行修补、精确 head CI 和独立 Agent 复审；全部门禁通过后由主 Agent 自动合并，只在明确的人类关口或用户当轮说“不要合并”时暂停。单一所有者、多 Agent 共用一个 GitHub 账号时，独立性是任务角色隔离，不伪造自我 Approval。

`开始做：…`、`搞定 Issue`、`把当前 Issue 跑完` 或显式调用 `agent-issue-loop` 用于一个具体交付 Issue。主 Agent 从验收就绪性开始，完成实现和本地验证，建立一个主 PR，再把该 PR 交给 `agent-pr-loop` 做对抗式审查、修补、CI 与自动合并；最后回读合并结果、逐项核对验收条件并正常关闭 Issue。整个过程只有一个主协调者，不要求用户在 Agent 之间传话。默认一个交付 Issue 对应一个主 PR；能独立合并的范围会先拆成关联子 Issue 和明确依赖顺序。

PR 循环只推送该 PR 的精确 head。GitHub 服务器端合并成功后只回读、验证和按需 fetch，不再推送已合并 head 或陈旧的本地 main；禁止用 `git push --all`、`git push --mirror`、合并后 force-push 或批量 tag push 作为收尾。
...
Claude Code 对应使用 `/integrate` 命令（由 `~/.claude/commands/integrate.md` 提供）：
...
Codex 的自定义 Slash Prompt 是其保留的兼容机制，官方已标记为 deprecated，并且只适用于 CLI 和 IDE 扩展；Claude Code 的 `/integrate` 是普通的用户自定义命令，不是 deprecated 入口。两者都只是快捷外壳：真正的全局、跨客户端工作流仍由 Skill 和“合并收尾”自然语言意图承担；ChatGPT 桌面端直接输入“合并收尾”即可。

### 三层配置分别负责什么

| 层级 | 作用 | 是否安装后自动完成 |
|---|---|---|
| Skills | `agent-project-bootstrap` 负责仓库政策、初始化和任务发现；`agent-issue-loop` 完成一个 Issue 从就绪、实现到合并验证与关闭；`agent-pr-loop` 完成一个 PR 的评论、复审、CI 与自动合并；三者同时安装到 Codex/ChatGPT 与 Claude Code | 是 |
| 全局 Prompt / 命令 | Codex `/prompts:integrate`（deprecated 兼容入口）；Claude Code `/integrate` | 是 |
| 全局规则文件 | Codex `AGENTS.md`；Claude Code `CLAUDE.md`。记住个人通用偏好：不要求 Issue 编号、识别短语、遇到歧义再问 | 仅使用 `--with-global-rule` 时 |
| 仓库规则文件 | Codex `AGENTS.md`；Claude Code `CLAUDE.md`。保存 Project 地址、精确状态名、测试命令和常规授权边界 | 每个仓库 bootstrap 时 |
...
因此，bootstrap 完成后，Agent 才能依据仓库规则自动执行已选任务内的常规动作。仅仅安装 Skill 不会触发合并；明确进入 `agent-issue-loop` 或 `agent-pr-loop` 后，满足精确 head 审查与 CI 门禁的单个 PR 才会自动合并。部署、删除、扩大需求范围及其他人类关口不会因此获得授权。执行工具本身如果出现平台授权提示，仍需用户批准。
...
| 稳定的 agent 与工程规则 | 仓库 `AGENTS.md`（Codex）或 `CLAUDE.md`（Claude Code） |
...
在当前机器的其他项目中不需要重复安装 Skills。进入那个仓库，调用 `@agent-project-bootstrap` 或 `$agent-project-bootstrap`（Claude Code 中直接描述需求）即可。每个仓库仍需单独初始化，因为技术栈、测试命令、忽略文件和分支策略都不同。
...
上面的命令假设安装到 Codex/ChatGPT；安装到 Claude Code 时把 `~/.codex` 换成 `~/.claude`。它把常用 Issue/PR 元数据写到被 Git 忽略的 `.codex/cache/github-snapshot.json`。Agent 可以在只读分析时先读取新鲜缓存；在创建、关闭、合并或需要最新状态前仍必须访问 GitHub。详见 [本地缓存](docs/local-cache-and-offline.md)。
...
python3 ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py skills/agent-issue-loop
python3 ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py skills/agent-pr-loop
