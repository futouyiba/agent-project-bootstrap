...
- 一个可安装的 Codex/ChatGPT/Claude Code Skill；
- 新项目和存量项目的交互式初始化流程；
...
默认安装到 Codex/ChatGPT 时，安装器把 Skill 写入 `$CODEX_HOME/skills`，把快捷 Prompt 写入 `$CODEX_HOME/prompts/integrate.md`；未设置 `CODEX_HOME` 时使用 `~/.codex`。安装到 Claude Code 时，Skill 写入 `~/.claude/skills`，slash 命令 `/integrate` 写入 `~/.claude/commands/integrate.md`，全局规则写入 `~/.claude/CLAUDE.md`；未设置 `CLAUDE_CONFIG_DIR` 时使用 `~/.claude`。如果已经安装旧版 Skill，旧目录会先被重命名为带时间戳的备份；不同内容的同名 Prompt 或命令也会先备份。再次使用 `--with-global-rule` 会升级这个项目自己管理的规则块，不会覆盖其他全局说明。
...
| 层级 | 作用 | 是否安装后自动完成 |
|---|---|---|
| Skill | 理解初始化、日常流程和可选 `gh-aw` 安装流程；同一份 Skill 同时安装到 Codex/ChatGPT 与 Claude Code | 是 |
...
在当前机器的其他项目中不需要重复安装 Skill。进入那个仓库，调用 `@agent-project-bootstrap` 或 `$agent-project-bootstrap`（Claude Code 中直接描述需求）即可。每个仓库仍需单独初始化，因为技术栈、测试命令、忽略文件和分支策略都不同。
