...
3. Reconcile state from current evidence: a draft PR keeps its linked Issue `In progress`; a non-draft PR ready for formal review puts it in `In review`. `Ready for review` is a PR stage, not an Issue status. Never dispatch an implementer solely for a metadata update.
...
5. If a pull request meets every repository merge gate and policy says `qualified_auto_merge`, enable auto-merge or enter the merge queue. Never deploy or publish.
6. If nothing is actionable, end this heartbeat quietly. Do not ask the user to copy status from another conversation.
7. Notify the user only when a documented human gate is reached or the configured retry limit is exhausted. First record a concise blocker and the exact decision needed on the linked Issue or PR.
