...
replacing any workflow. A failed replacement rolls the whole set back; after process
interruption, the next `--apply` recovers the old set under the repository
transaction lock before replanning and retrying. A read-only preview fails
closed while such a manifest is pending. Any edited legacy file remains a
conflict, and a legacy live profile requires manual review rather than a silent
rewrite. Never combine legacy migration with `--live`; migrate and validate the
new staged profile first.
...
- Keep Project credentials outside the agent workflow. The supervisor may only
  ...
  fixed generated Project URL and existing item, and writes only its
  `Status: In review` option using the dedicated Project token.
