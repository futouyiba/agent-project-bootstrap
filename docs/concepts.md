...
Backlog 是候选工作的集合或队列。它可以包含处于 Backlog 状态的 Issues，也可以包含尚未转成 Issue 的 Project draft items。Backlog 关注“还没有承诺现在做什么”；Issue 关注“这一项工作具体是什么”。Backlog 不等于高优先级。

不要创建一份 Backlog 文档，再把同样的内容复制成 Issues。明确事项直接创建 Issue 并加入 Project；不确定想法可以先成为 Project draft item，澄清后再转换为 Issue。
